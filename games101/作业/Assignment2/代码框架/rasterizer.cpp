...
{   
    ...
    return AC.cross(AP).z() > 0 && CB.cross(CP).z() > 0 && BA.cross(BP).z() > 0 || AC.cross(AP).z() < 0 && CB.cross(CP).z() < 0 && BA.cross(BP).z() < 0;
}
    ...
    for(int x = min_x; x < max_x;x++)
        for (int y = min_y; y < max_y; y++)
        {
            if (insideTriangle(x + 0.5, y + 0.5, t.v))
            {
                ...
                Vector3f pixel = { float(x),float(y),z_interpolated };
