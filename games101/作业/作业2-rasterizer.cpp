...
{   
    ...
    float z0 = AB.cross(AP).z(),z1 = BC.cross(BP).z(),z2 = CA.cross(CP).z();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    int min_x = std::min(v[0].x(),std::min(v[1].y(),v[2].y()));
        ...
        for(int y = min_y;y < max_y;y++)
        {
            if(insideTriangle(x+0.5,y+0.5,t.v))
            {
                ...
                if(z_interpolated < depth_buf[get_index(x,y)])
                {
                    ...
                    Vector3f pixel = {(float)x,(float)y,z_interpolated},color = t.getColor();
