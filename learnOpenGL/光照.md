...
当描述一个表面时，我们可以为四个光照分量（环境光、漫反射、镜面光照、反光度）定义一个材质颜色（Material Color），以此对表面的颜色进行细粒度控制；
...
![image-20240125140002078](C:\Users\sunha\Desktop\book\ReadingNotes\learnOpenGL\光照.assets\image-20240125140002078.png)

物体过于光亮

## 光的属性

物体的每个材质属性对每一个光照分量都返回了最大的强度
...
当光源处于一个很远的地方时，来自光源的每条光线都会近似平行；

由于光线是平行的，所以光源的位置和物体的位置对效果的影响不大，可以使用光源的方向向量来模拟光照，不需要位置向量；
...
* SpotDir：聚光所指向的方向
* Φ：指定了聚光半径的切光角，落在这个角度之外的物体都不会被这个聚光所照亮
* θ：lightDir和SpotDir之间的夹角

计算lightDir和spotDir之间的夹角与切光角
