...
ThreadPool* threadPoolCreate(int min,int max,int queueSize)
{
    do
    {
        ThreadPool* pool = (struct ThreadPool*)malloc(sizeof(struct ThreadPool));
...
void* worker(void* arg)
{
    ...
    while(1)
    {
        ...
        //当任务队列为空时，阻塞队列
        while(!pool->shutdown && !pool->queueSize)
        {
        ...
        //取出任务
        Task task;
        task.function = pool->taskQ[queueFront].function;
        task.arg = pool->taskQ[queueFront].arg;
