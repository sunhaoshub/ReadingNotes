...
* 能够被多个线程访问并且保证数据安全性，如初始化某个对象，但这个对象只能初始化一次
* 被call_once函数修饰的函数（回调函数f）只能被调用一次

```C++
void call_once(std::once_flag& flag,Callable&& f,Args&... args);
/*
	需要确保flag能够被多个线程同同时访问到
*/
```
...
void func()
{
	cout << "call_once func()..." << endl;
}

void dosomething(string name,int age)
{
	static int num = 1;
	call_once(m_flag, func);
	cout << "dosomething().... name = "<<name<<",age = "<<age << ",num = " << num++ << endl;
}

int main()
{
	thread t1(dosomething, "a", 10000);
	thread t2(dosomething, "b", 20000);
	thread t3(dosomething, "c", 30000);
...
* 可能出现多个线程同时创建实例的情况，应对措施：①上锁②call_once函数
...
void myFunc(string name)
{
	...
	cout << "my name is : " << Singleton::getInstance()->getName() << endl;
}
...
在wait位置阻塞之前会释放互斥锁，在wait位置解除阻塞之后需要重新抢夺互斥锁，加锁成功之后才能继续向下执行
...
public:
	...
	void take()
	{
		int num = taskQueue.front();
		cout << "取出任务：" << num << ",线程id：" << this_thread::get_id() << endl;
		taskQueue.pop();
	}
