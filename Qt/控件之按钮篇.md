...
半选中状态（选择框中是实心方块）：子节点没有全部选中，只有一部分被选中，这种状态默认是没有的，需要通过setCheckState函数来打开
    ...
    ui(new Ui::MainWindow)
{
    ...
    //父选择框全选
    connect(ui->parentBox,&QCheckBox::clicked,this,[=](bool bl){
        if(bl)
        {
            ui->AcheckBox->setChecked(1);
            ui->BcheckBox->setChecked(1);
            ui->CcheckBox->setChecked(1);
            ui->DcheckBox->setChecked(1);
            ui->EcheckBox->setChecked(1);
        }
        else
        {
            ui->AcheckBox->setChecked(0);
            ui->BcheckBox->setChecked(0);
            ui->CcheckBox->setChecked(0);
            ui->DcheckBox->setChecked(0);
            ui->EcheckBox->setChecked(0);
        }
    });

    ui->parentBox->setTristate(1);

    connect(ui->AcheckBox,&QCheckBox::stateChanged,this,&MainWindow::statusChanged);
    connect(ui->BcheckBox,&QCheckBox::stateChanged,this,&MainWindow::statusChanged);
    connect(ui->CcheckBox,&QCheckBox::stateChanged,this,&MainWindow::statusChanged);
    connect(ui->DcheckBox,&QCheckBox::stateChanged,this,&MainWindow::statusChanged);
    connect(ui->EcheckBox,&QCheckBox::stateChanged,this,&MainWindow::statusChanged);
}
...
void MainWindow::statusChanged(int state)
{
    ...
    if(m_number == 5)
    {
        ui->parentBox->setCheckState(Qt::Checked);
    }
    else if(m_number == 0)
    {
        ui->parentBox->setCheckState(Qt::Unchecked);
    }
    else
        ui->parentBox->setCheckState(Qt::PartiallyChecked);
}
```
